...
def add_output_path(logger, output_log_dir, prefix=None):
    """Add a second file handler so the same log is mirrored to another folder.

    Usage: after get_logger(...), call add_output_path(logger, debug_dir, prefix='train')
    to also mirror logs into debug/<branch>/<scene>/train_<timestamp>.log.
    """
