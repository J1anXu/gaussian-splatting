...
def merge_opt(
    ...
    if (vis_filter_list is not None and radii_list is not None and len(vis_filter_list) == len(radii_list)):
        vis_list = []
        offset = 0
        for vf, radii in zip(vis_filter_list, radii_list):
            num_r = radii.shape[0]
        ...
        if len(vis_list) > 0:
            final_visibility_filter = torch.cat(vis_list, dim=0).unsqueeze(1)
        else:
            final_visibility_filter = torch.empty((0, 1), dtype=torch.long, device=device)
