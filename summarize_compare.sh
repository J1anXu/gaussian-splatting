...
def parse_train(train_log: Path | None, extra_logs: list[Path] | None = None) -> tuple[str, str, str, float | None, bool, bool, int | None, int | None]:
    ...
    if peak is None:
        # current format: per-iter peak stored under 'rsv'
...
def summarize_debug_scene(dataset: str, scene_dir: Path, branch: str) -> Summary:
    scene = scene_dir.name
    metrics = parse_debug_metrics(latest_log(scene_dir, "metrics"))
    train_log = latest_log(scene_dir, "train")
    # Structured logger output (logs/train/<branch>/<scene>/*.log) has the
    # 'rsv' / 'alloc' dict lines; debug/train.log is stdout/tqdm + final
    # time line only.
    structured_logs = sorted((SCRIPT_DIR / "logs" / "train" / branch / scene).glob("*.log"))
    points, blocks, peak_mem, duration, train_complete, crashed, train_points_raw, train_blocks_raw = parse_train(train_log, structured_logs)
...
def summarize_output_scene(branch: str, dataset: str, scene_root: Path) -> Summary:
    ...
    # Also fall back to the repo-local structured logger output for peak mem
    # / rsv values when the output tree lacks them.
    structured_logs = sorted((SCRIPT_DIR / "logs" / "train" / branch / scene).glob("*.log"))
    points, blocks, peak_mem, duration, train_complete, crashed, train_points_raw, train_blocks_raw = parse_train(train_log, structured_logs)
...
def collect_debug_branch(branch: str) -> list[Summary]:
    ...
    return [summarize_debug_scene(dataset, scene_dir, branch) for _, scene_dir, dataset in candidates.values()]
