...
用法:
    python3 plot/CapGS/plot_mem_bar.py data/bicycle_metrics.csv
    python3 plot/CapGS/plot_mem_bar.py data/bicycle_metrics.csv --name bicycle --bins 30
"""
import argparse
import csv
...
def main():
    ...
    # bin
    bin_edges = np.linspace(iters[0], iters[-1], args.bins + 1)
    bin_centers = []
    bin_alloc = []
    bin_frag = []
    for i in range(args.bins):
        mask = (iters >= bin_edges[i]) & (iters < bin_edges[i + 1])
        if i == args.bins - 1:
            mask |= (iters == bin_edges[i + 1])
        if mask.sum() == 0:
            continue
    ...
    ax.bar(bin_centers, bin_alloc, width=width, label="allocated", color="#FF9800")
    ax.bar(bin_centers, bin_frag, width=width, bottom=bin_alloc, label="fragmented (rsv − alloc)", color="#FF9800", alpha=0.3)
