"""
绘制 rsv_gb-step 和 alloc_gb-step 曲线。

用法:
    python3 plot/gs-scale/plot_mem.py data/bicycle_metrics.csv
    python3 plot/gs-scale/plot_mem.py data/bicycle_metrics.csv --name bicycle
"""
import argparse
import csv
...
def main():
    ...
    ax.plot(steps, rsv, label="reserved (rsv_gb)", alpha=0.8, linewidth=0.5)
    ax.plot(steps, alloc, label="allocated (alloc_gb)", alpha=0.8, linewidth=0.5)
    ax.set_xlabel("Step")
