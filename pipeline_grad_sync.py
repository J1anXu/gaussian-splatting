...
class PipelinedGradSync:
    ...
    def kick_async_d2h(self, submodel: GaussianModel, submodel_id: int, render_pkg: dict, sub_viewspace_point_tensor):
    ...
    def _make_pending(self, sm, sm_id, idx, gpu_grads, cpu_params,
        ...
        def _do():
            ev_all = tracer.begin(f"worker[{sm_id}]", tier="worker", block_id=sm_id)
    ...
    def flush(self):
        ...
        if self._worker_thread is not None:
            ev = tracer.begin(f"wait[{self._running_block_id}]", tier="block", block_id=self._running_block_id)
