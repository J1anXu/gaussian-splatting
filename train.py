...
def training(dataset, opt, pipe, saving_iterations, debug_from, res):
    ...
    log_kv(LOGGER, "training_runtime_options", {
        ...
        "merge_fast_max_k": getattr(config, "MERGE_FAST_MAX_K", 16),
        "merge_fast_validate": getattr(config, "MERGE_FAST_VALIDATE", False),
        "merge_fast_validate_every": getattr(config, "MERGE_FAST_VALIDATE_EVERY", 1),
        "merge_fast_validate_until": getattr(config, "MERGE_FAST_VALIDATE_UNTIL", 750),
        "merge_fast_validate_atol": getattr(config, "MERGE_FAST_VALIDATE_ATOL", 1e-5),
        "merge_fast_validate_bg_atol": getattr(config, "MERGE_FAST_VALIDATE_BG_ATOL", getattr(config, "MERGE_FAST_VALIDATE_ATOL", 1e-5)),
        "merge_fast_validate_fatal": getattr(config, "MERGE_FAST_VALIDATE_FATAL", False),
